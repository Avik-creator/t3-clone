  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 w-full">
      <div className="w-full max-w-3xl space-y-6 md:space-y-8">
        ...
        <div className="flex flex-wrap gap-2 w-full justify-center md:justify-start">
          {CHAT_TAB_MESSAGE.map((tab, index) => (
            <Button
              ...
              className="w-[100px] md:w-27.5 justify-start text-xs md:text-sm"
            >
              {tab.icon}
        ...
        <div className="space-y-2 md:space-y-3 w-full min-h-[200px] md:min-h-60">
          {CHAT_TAB_MESSAGE[activeTab].messages.map((message, index) => (
            <div key={index}>
              <button
                onClick={() => onMessageSelect(message)}
                className="w-full text-left text-xs md:text-sm text-muted-foreground hover:text-primary transition-colors duration-300 ease-in-out py-2 px-1"
              >
                {message}
              </button>
